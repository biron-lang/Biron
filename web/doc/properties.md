...
Because every property is a compile-time value or a type, an `of` result may stand wherever a constant or a type annotation is expected, including an array length, a `const` binding, or a parameter type. `of` sits at the same precedence as `as!` and `is`. See **Expressions** for the full table.
